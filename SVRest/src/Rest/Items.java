...
public class Items {
	...
	public String Get_Users() throws JSONException{
          ...
          try {
			 ...
			 JSONArray items = jsonResponse.getJSONArray("docs");
			 JSONArray all_items = new JSONArray()  ; 
			 
			 for(int i=0 ; i<items.length() ; i++){
				 JSONObject j = new JSONObject() ;  
				 j.put("name", items.getJSONObject(i).getString("name"));
				 j.put("price", items.getJSONObject(i).getString("price"));
				 j.put("expiry_date", items.getJSONObject(i).getString("expiry_date"));
				 j.put("calories", items.getJSONObject(i).getString("calories"));
				 all_items.put(j) ; 
			 }
			 
			 return all_items.toString() ; 
